...
Auto_Aimer::Auto_Aimer(ros::NodeHandle &_nh, int &_loop_rate,
                       int &_lost_time_threshold)
    : nh(_nh),
      loop_rate(_loop_rate),
      lost_time_threshold(_lost_time_threshold) {
  ...
  // init subscriber
  sub_emergence = nh.subscribe("/emergency_stop_flag", 1,
                               &Auto_Aimer::emergence_callback, this);
  sub_connection = nh.subscribe("/connection_status", 1,
                                &Auto_Aimer::connection_callback, this);
  sub_teleop =
      nh.subscribe("/teleop_flag", 1, &Auto_Aimer::teleop_callback, this);
  sub_target =
...
void Auto_Aimer::joy_callback(const sensor_msgs::Joy::ConstPtr &msg) {
  cmd_ELV += -msg->axes[7] / 10;
  cmd_TRN += msg->axes[6] / 360;
...
void Auto_Aimer::autoAimer() {
  cmd_ELV =
      -0.4475 * pow(target_distance, 6) + 5.2814 * pow(target_distance, 5) -
      24.424 * pow(target_distance, 4) + 56.043 * pow(target_distance, 3) -
      67.331 * pow(target_distance, 2) + 44.159 * target_distance - 10.077 +
      offset;
