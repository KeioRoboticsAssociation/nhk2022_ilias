...
class Rosconnector():
    ...
    def __init__(self):
        ...
        self.init_sub = rospy.Subscriber(
            "hard_init", Empty, self.hardinit_callback)
        self.limit_pub = rospy.Subscriber(
            "rcv_serial_11", Float32MultiArray, self.limit_switch_callback)
        self.current_sub = rospy.Subscriber(
            "current_cmd", Float32MultiArray, self.current_sub_callback)
    ...
    def limit_switch_callback(self, msg):
        if msg.data[0] == 0:
            if msg.data[1] == 0:
                ...
                rospy.sleep(0.1)
                self.send_rogilink_b(HardId.TURN_ANGLE.value, 0x02, 0)
                self.send_rogilink(HardId.TURN_ANGLE.value, 0x09, 1.3, 0)
            elif msg.data[1] == 4:
    ...
    def hardinit_callback(self, msg):
        ...
        if self.limit_switch_array[0] == 1:
            ...
            self.send_rogilink(HardId.ELEVATION_ANGLE.value, 0x06, -0.5, 0)
            rospy.logwarn("hard init command")
